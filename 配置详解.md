...
| `default_summarization_model` | 字符串 | None | 用于生成摘要的默认模型（当 default_reduction_mode 为 summarization 时必需） |
...
**⚠️ 重要：使用 summarization 模式时，必须配置 `default_summarization_model` 或在 `model_mappings` 中配置 `summarization_model` 字段！**
...
context:
  ...
  default_summarization_model: openai/gpt-3.5-turbo  # 全局默认摘要模型
...
model_mappings:
  - display_name: openai/gpt-4
    ...
    context_config:
      ...
      summarization_model: openai/gpt-3.5-turbo  # 模型级别配置，覆盖全局
...
model_mappings:
  ...
  - display_name: openai/gpt-4
    ...
    context_config:
      ...
      max_tokens: 8000
      reduction_mode: summarization
      summarization_model: openai/gpt-3.5-turbo  # 必须指定！用便宜的模型做摘要
      memory_zone_enabled: true
...
| 配置项 | 类型 | 必需 | 说明 |
|--------|------|------|------|
| `max_turns` | 整数 | 否 | 该模型的最大对话轮次 |
| `max_tokens` | 整数 | 否 | 该模型的最大 Token 数 |
| `reduction_mode` | 字符串 | 否 | 缩减策略：truncation, sliding_window, summarization |
| `summarization_model` | 字符串 | 是* | 用于生成摘要的模型（**当 reduction_mode 为 summarization 时必需**） |
| `memory_zone_enabled` | 布尔 | 否 | 是否启用记忆区存储摘要 |

*当 `reduction_mode` 为 `summarization` 时，`summarization_model` 为必需字段
...
context:
  ...
  default_reduction_mode: sliding_window
  default_summarization_model: openai/gpt-3.5-turbo  # 如果使用 summarization 模式，需要配置
  summarization_prompt: |
    你是一个对话摘要助手。请简洁地总结以下对话，
    保留关键信息、用户意图和重要上下文。
    将摘要控制在 {max_tokens} 个 Token 以内。
...
model_mappings:
  ...
  - display_name: openai/gpt-4
    ...
    context_config:
      ...
      max_tokens: 8000
      reduction_mode: summarization
      summarization_model: openai/gpt-3.5-turbo  # 必须指定！
      memory_zone_enabled: true
...
| `summarization_model is not configured` | 使用 summarization 模式但未配置摘要模型 | 在 context 中添加 default_summarization_model 或在 model_mappings 的 context_config 中添加 summarization_model |
| `default_summarization_model is not configured` | 全局使用 summarization 模式但未配置默认摘要模型 | 在 context 中添加 default_summarization_model |
